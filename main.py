...
import jetson_utils
import time
net = jetson_inference.detectNet("/home/vqbg/jetson-inference/python/training/detection/ssd/models/reserve_cloth/ssd-mobilenet.onnx", labels ="/home/vqbg/jetson-inference/python/training/detection/ssd/models/reserve_cloth/labels.txt",input_blob="input_0",output_cvg="scores",output_bbox="boxes", threshold=0.5)
#camera = jetson_utils.gstCamera(640, 480, "csi://0?framerate=30/1")
...
class camera1(QThread):
    ...
    def run(self):
        ...
        while True:
            ...
            #set_data=self.load_data() cap.isReady() display.IsOpen() and
            if (globalvar.nomodel==0):
                #keyboard.wait("enter")
                img_cuda, width, height = camera.CaptureRGBA()
                detections = net.Detect(img_cuda, width, height)
                #time.sleep(1/10)
                found_NG = any(d.ClassID == 2 for d in detections)
                if found_NG:
                    print("NG")
                ...
                for r in range(globalvar.qtycam1):
                    ...
                    #thres_tem= 0.95
                    #loc = np.where(result_gpu>= thres_tem)
                    #print( "loc", loc)
                    #for pt in zip(*loc[::-1]):
