            ...
            if cap.isReady():            
                ...
                for r in range(globalvar.qtycam2):
                    ...
                    if r== 1:
                        read_image = cv2.imread('picture2_{}.jpg'.format(r+1))
    			              if read_image is None:
        			              print("Lỗi: Không tìm thấy ảnh mẫu picture2_2.jpg")
    			              else:
        		            # Resize template cho khớp với ROI hiện tại
        			              if read_image.shape != imgCropped.shape:
            				            read_image = cv2.resize(read_image, (imgCropped.shape[1], imgCropped.shape[0]))
                            # Chuyển sang grayscale
        			              gray_current = cv2.cvtColor(imgCropped, cv2.COLOR_BGR2GRAY)
        			              gray_template = cv2.cvtColor(read_image, cv2.COLOR_BGR2GRAY)
				                    # ====================== HÀM TÌM TÂM HÌNH CHỮ NHẬT ======================
        			              def find_rect_center(gray_img, min_area=300):
                                ...
                                for cnt in contours:
                                    ...
                                    if len(approx) == 4:                                 # Chỉ lấy hình 4 cạnh
                                        x, y, w, h = cv2.boundingRect(cnt)
                                        aspect_ratio = float(w) / h
                                        if 0.5 < aspect_ratio < 2.0:                     # Cho phép chữ nhật/vuông
                                            if area > best_area:
                                                best_area = area
                                                best_contour = approx
                                                M = cv2.moments(cnt)
                                                if M["m00"] != 0:
                                                    ...
                                                    cy = int(M["m01"] / M["m00"])
                                                    best_center = (cx, cy)
                            return best_center, best_contour, best_area
                            # Tìm tâm trên ảnh mẫu (template)
                            template_center, template_contour, template_area = find_rect_center(gray_template)
        
                            # Tìm tâm trên ảnh real-time
                            current_center, current_contour, current_area = find_rect_center(gray_current)

                            # ====================== TÍNH ĐỘ LỆCH & QUYẾT ĐỊNH ======================
                            diff_bit = 1
                            center_ok = 0
                            if current_center is not None and template_center is not None:
                                # Tính độ lệch
                                dx = current_center[0] - template_center[0]
                                dy = current_center[1] - template_center[1]
                                offset_distance = abs(dx) + abs(dy)        # hoặc np.hypot(dx, dy)

                                # Vẽ debug
                                cv2.drawContours(imgCropped_proc, [current_contour], -1, (0, 255, 0), 2)
                                cv2.circle(imgCropped_proc, current_center, 8, (0, 0, 255), -1)
                                cv2.putText(imgCropped_proc, f"Center: {current_center}", 
                                    (current_center[0]+15, current_center[1]-15),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
            
                                cv2.drawContours(read_image_proc, [template_contour], -1, (0, 255, 0), 2)
                                cv2.circle(read_image_proc, template_center, 8, (0, 0, 255), -1)

                                # Kiểm tra ngưỡng
                                threshold = globalvar.read_database[r,44]
                                if offset_distance <= threshold:
                                    diff_bit = 0
                                    center_ok = 1
                                    client.write_coil(8992, True)
                                else:
                                    diff_bit = 1
                                    center_ok = 0
                                    client.write_coil(8992, False)
                
                                print(f"Độ lệch tâm: dx={dx}, dy={dy} | Tổng lệch={offset_distance} | Ngưỡng={threshold}")
                            else:
                                # Không tìm thấy hình chữ nhật ở một trong hai ảnh
                                print("Không phát hiện hình chữ nhật!")
                                client.write_coil(8992, False)
                    ...
                    if globalvar.read_database[r,1] == globalvar.pictureid:
                        ...
                        if r==1:
                            self.send_actualpixel2.emit(current_area)      # Diện tích hiện tại
                            self.send_actualdifferent2.emit(diff_bit)      # 0=OK, 1=NG
                            self.send_actuallengh2.emit(center_ok)         # Tâm hợp lệ

                            # Hiển thị stack ảnh debug
                            imgstack = self.stackImages(1, [
                                [imgCropped_proc, read_image_proc, 
                                 cv2.cvtColor(gray_current, cv2.COLOR_GRAY2BGR), 
                                 cv2.cvtColor(gray_template, cv2.COLOR_GRAY2BGR)]
                            ])))
                            imgstack=cv2.cvtColor(imgstack,cv2.COLOR_BGR2RGB)
                            imgstack =QImage(imgstack,imgstack.shape[1],imgstack.shape[0],imgstack.strides[0],QImage.Format_RGB888)
...
"")
    ...
    def update_fun(self):
        ...
        connection = mysql.connector.connect(host='localhost',
                                            ...
                                            user ='vqbg',
                                            password='vqbg123!')
        if (self.runmodel ==666666)&(self.pictureid >0):
            try:              
    ...
    def copydatabtn_fun(self):
        if (self.runmodel ==666666)&(self.pictureid >0)&(self.copymodel >600000):
            try:
